<script lang="ts">
    ...
    let marketFixed = urlConverter.fixMarket(market);

    // ?? will prevent undefined appear in string
    let localLanguage = (
        localLanguageLeft ?? localLanguageRight
    )?.toLowerCase();
    ...
    const beta = urlConverter.isMarketInBeta(marketFixed);

    const marketPath = `/content/guxeu${urlConverter.betaString(beta)}/${marketFixed}`;
    const marketLocalLangPart = `/${urlConverter.fixLocalLanguage(localLanguage, marketFixed, true) || marketFixed}_${marketFixed}`;
